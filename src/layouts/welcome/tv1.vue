<template>
  ...
  <div class="relative flex flex-wrap min-h-screen " >
    <div class=" flex w-full h-20 bg-blue-500 text-gray-100 p-1 absolute left-0 top-0 right-0" >
    ...
    <Transition name="slide-fade" >
      <div v-if="toggleLoginForm && meetings.length" class="absolute right-0 left-0 top-20 bottom-20" >
        <table class="table-fixed w-full min-h-full bg-gray-100 border border-gray-200" >
          <tr class=" border-b border-gray-200 h-8">
            ...
            <th class="font-btb-black text-center text-3xl  p-4" >ខ្លឹមសារ</th>
          ...
          <tr v-for="(meeting,index) in meetings" :key="index" :class="( index % 2 ? ' bg-gray-100 ' : ' bg-gray-200 ' ) " >
            ...
            <td class="text-left py-2" >
              ...
              <div class="font-btb-black my-4 whitespace-nowrap overflow-hidden overflow-ellipsis text-3xl  text-gray-800" >{{  meeting.objective }}</div>
              <div class="font-btb-black text-2xl text-gray-600" >{{ meeting.listMembers.length > 0 ? "ក្រោមអធិបតីភាព ៖ " + meeting.listMembers.filter( (memberInList) => memberInList.role == 'lead_meeting' && memberInList.group == 'leader' ).map( (memberInList) => memberInList.member.lastname + " " + memberInList.member.firstname ).join(' - ') : '' }}</div>
            </td>
            ...
            <td class="w-2/12 text-center" >
              <div :class=" ' xl:text-2xl  xl:p-3 w-3/4 mx-auto rounded-full text-center p-1 text-white font-bold ' + ( statuses.find( (s) => s.code == meeting.status ).color )" >{{ statuses.find( (s) => s.code == meeting.status ).label }}</div>
            </td>
          </tr>
        </table>
      </div>
    ...
    <div class=" fixed left-0 right-0 bottom-0 bg-blue-500 flex h-20">
      <div class="
