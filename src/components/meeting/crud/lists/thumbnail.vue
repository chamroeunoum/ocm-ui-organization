<template>
  <div class="relative " >
            ...
            <div class="content " >
              ...
              <div class="flex flex-wrap my-2" >
                <div class="w-full py-2" >
                  <div class="w-full text-left leading-6 my-2 text-md font-pvh" v-html="applyTagMark( record.objective )" ></div>
                  <div class="w-full flex my-1" >
                    <div class="w-1/2 text-left text-gray-600 mr-2 leading-4 font-moul text-vcb-xs" >{{ $toKhmer( record.date ) }}</div>
                    <div class="w-1/2 text-right text-gray-600 leading-4 font-moul text-vcb-xs" >{{ Array.isArray( record.rooms ) && record.rooms.length > 0 ? record.rooms.map( o => o.name ).join( ' , ' ) : '' }}</div>
                  </div>
                  <div class="w-full flex my-1" >
                    <div class="w-1/2 text-left text-gray-600 mr-2 leading-4 font-moul text-vcb-xs" >{{$toKhmer( record.start + ' - ' + record.end ) }}</div>
                    <div class="w-1/2 text-right text-gray-600 leading-4 font-moul text-vcb-xs" >{{ record.type != undefined && record.type != null && record.type.id > 0 ? record.type.name : '' }}</div>
                  </div>
                </div>
              </div>
            ...
            <n-tooltip trigger="hover">
              <template #trigger>
                <n-popselect 
                  ...
                  @update:value="goTo(1)"
                >
                  <div class="cursor-pointer font-pvh rounded-full p-2 px-4 border border-gray-200 text-blue-600 leading-4" >{{ $toKhmer( pagination.perPage ) }}</div>
                </n-popselect>
              </template>
    ...
    <div class="fixed left-0 top-0 right-0 bg-white flex title-bar border-b border-gray-200 p-2 h-12 ">
