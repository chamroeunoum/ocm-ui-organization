            ...
            <div class="flex justify-center flex-wrap w-full" >
              <div v-for="(person,index) in people" :key="index" class="p-2" >
                <div class="p-2 border border-gray-200 rounded-lg w-60 cursor-pointer hover:bg-gray-100 duration-300 relative" >
                  ...
                  <div v-if="person.countesies != undefined && person.countesies.length" class="member_countesies text-gray-700 font-moul text-center text-xs leading-6 " >{{ person.countesies.map( (p) => p.name ).join(' ') }}</div>
                  <div class="member_name  text-gray-700 w-full mb-2 font-moul text-center" >{{ person.lastname + " " + person.firstname }}</div>
                  <div v-if="person.organizations != undefined && person.organizations.length" class="member_organizations mb-2 text-gray-500 text-xs leading-6 text-center" >{{ person.organizations.map( (p) => p.name ).join(' - ') }}</div>
                  <div v-if="person.positions != undefined && person.positions.length" class="member_positions text-gray-500 text-xs  mb-2 text-center" >{{ person.positions.map( (p) => p.name ).join(' - ') }}</div>
