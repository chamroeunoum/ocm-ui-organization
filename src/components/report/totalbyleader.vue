<template>
  <div class="w-full" >
    <div class="w-full leading-9 font-moul -mt-12 mb-2 text-left pl-16 z-45 print:hidden" v-html="model.title" ></div>
    <div class="w-full text-left flex p-4 bg-gray-100 z-40 print:hidden">
      ...
      <n-tooltip trigger="hover">
        <template #trigger>
          <n-select class="flex-grow ml-4" @update:value="updateLeader" :options="optionLeaders" filterable placeholder="សូមជ្រើសរើសថ្នាក់ដឹកនាំកិច្ចប្រជុំ" />
        </template>
        សូមជ្រើសរើសថ្នាក់ដឹកនាំកិច្ចប្រជុំ ដើម្បីផលិតរបាយការណ៍
      </n-tooltip>
    ...
    print:absolute print:top-0 print:left-0 print:right-0 
    ...
    " >
      <div class="page relative w-a4w h-a4h my-8 mx-auto shadow-lg rounded-lg border border-gray-200 print:border-none print:rounded-none print:border-0 print:shadow-none print:my-0 print:mt-2  " style="page-break-after: always;" >
        <div class="sub-page relative p-8" >
          ...
          <!-- Header -->
          <div class="report-header " style="color: #042fa2; " >
            ...
            <div v-if="leader!=null" class="report-title font-moul mt-8 text-md" >សរុបរបាយការណ៍កិច្ចប្រជុំដឹកនាំដោយ<br/>{{ leader.people.lastname + ' ' + leader.people.firstname }}</div>
          </div>
          <!-- Objective -->
          
          <div v-if="leader!=null" class="report-content text-xs my-8 leading-8 text-left" >
            <table v-if="leader.meetings.length > 0 " class="vcb-table rounded-lg" >
              <tr class="vcb-table-headers" >
                <th class="vcb-table-header" >ល.រ</th>
                <th class="vcb-table-header">ខ្លឹមសារ</th>
              </tr>
              <tr v-for="(record, index) in leader.meetings" :key='index' class=" vcb-table-row" >
                ...
                <td class="main-cell vcb-table-cell flex flex-wrap justify-content relative" >
                  ...
                  <div v-show="false"  v-if="Array.isArray( record.leaders ) && record.leaders.length > 0" class="pr-4 flex text-gray-600" >
                    <n-tooltip trigger="hover">
                      <template #trigger>
                    ...
                    {{ record.leaders.map( o => o.name ).join( ' , ' ) }}
                  </div>
            ...
            <div v-if="leader.meetings.length > 0 " class="total-information my-4 w-full" >
              <div class="p-2 " >ចំនួនម៉ោងប្រើប្រាស់ដើម្បីប្រជុំ ៖ {{ leader != null ? ( parseInt( leader.totalSpentMinutes / 60 ) + ' ម៉ោង' ) : "0 ម៉ោង" }}</div>
              <div class="p-2 " v-if="leader != null " >
                ប្រភេទកិច្ចប្រជុំ ៖
                <div class="font-sr ml-4" v-for="(type,index) in leader.totalMeetingsByTypes" >{{ (index+1) }}. {{ type.type.name + " ចំនួន " + type.total + " ដង" }}</div>
              </div>
            </div>
          </div>
          <div class="created_by my-4 w-full bg-green-400 flex flex-col-reverse" >
            <div class="created_by w-1/3 absolute right-0 bottom-0 " >
              <div class="text-center p-2" >ថ្ងៃច័ន្ទ ទី០៤ ខែមីនា ឆ្នាំ ២០២៤</div>
              <div class="text-center h-28 w-28" ></div>
              <div class="text-center p-2 font-moul" >{{ reporter }}</div>
            </div>
          </div>
          <div v-if="leader==null" class=" text-gray-500 text-center place-items-center place-content-center w-full h-60 flex" >
            <div v-if="selectedLeader==null" class="text-xl" >សូមជ្រើសរើសថ្នាក់ដឹកនាំកិច្ចប្រជុំដើម្បីបង្កើតរបាយការណ៍</div>
            <div v-if="selectedLeader!=null" class="text-xl" >មិនមានរបាយការណ៍កិច្ចប្រជុំដែលដឹកនាំដោយ {{ selectedLeader.label }}</div>
          </div>
        </div>
      </div>
      <!-- <div class="page" >
...
export default {
  name: "TotalMeetingReportByLeader" ,
  components: {
    // Form
  },
  setup(){
    ...
    const model = reactive({
      name: 'reportbyleader' ,
      title: 'របាយការណ៍ តាមថ្នាក់ដឹកនាំកិច្ចប្រជុំ'
    })
    ...
    const optionLeaders = computed( () => {
      return leaders.value.length > 0 ? leaders.value.map( (o) => { return { label: o.lastname + ' ' + o.firstname , value: o.id } } ) : []
    })
    const selectedLeader = ref(null)
    function getLeaders(){
      store.dispatch('meeting/people',{
        page: 1 ,
        perPage : 1000 ,
        search : ''
      }).then( res => {
        if( res.data.ok ){
          leaders.value = res.data.records
        }else{
          notify.error({
            title: 'អានព័ត៌មាន' ,
            content: 'មានបញ្ហាអានព័ត៌មាន។' 
          })
        }
      }).catch( err => {
        console.log( err )
    ...
    function updateLeader(value , selection ){
      selectedLeader.value = selection
      fetchReportData()
    }

    function fetchReportData(){
      if( selectedLeader.value.value > 0 ){
        store.dispatch('meeting/totalByTypeByLeader',{
          id: selectedLeader.value.value
        }).then( res => {
          if( res.data.ok ){
            if( Array.isArray( res.data.result ) && res.data.result.length > 0 ){
              leader.value = res.data.result[0]
            }else{
              leader.value = null  
            }
          }else{
