<template >
  ...
  <div class="flex flex-wrap justify-center pb-4" >
    <div class="flex border-b border-gray-200 pb-2 w-full mb-4" >
      ...
      <div class="flex-grow font-moul text-xs leading-7 text-left ml-1 ">សរុបកិច្ចប្រជុំតាមក្រសួងស្ថាប័នប្រចាំត្រីមាស</div>
      <!-- <div class="font-bold flex-none w-20 text-right leading-6 text-blue-600 " v-html="total" ></div> -->
    </div>
    <div v-for="(organization,index) in listOrganization" :key="index" 
      ...
      @click="preview(organization)"
    >
      <div class="w-full text-center mb-4 " >
        <div class="font-moul text-xs mb-2" >{{ organization.name }}</div>
      </div>
      <div v-for="(meeting,index) in organization.totalMeetingsByTypes" :key="index" 
        class="w-full flex" >
      ...
      <div class="w-full flex text-green-500" >
        ...
        <div class="w-40 text-right " >{{  organization.total }}</div>
      </div>
      <!-- <div class="w-full flex text-green-500" >
        <div class="flex-grow text-left mb-4 " >សរុបរយះពេល</div>
        <div class="w-40 text-right " >{{  parseInt( organization.totalSpentMinutes / 60 ) }} ម៉ោង</div>
      </div> -->
    </div>
    <n-drawer v-model:show="drawer.show" width="90%" placement="right">
      <n-drawer-content >
        <div class="border-b border-gray-200 pb-4 font-moul mb-4 leading-8" >
          {{  selectedOrganization != null ? selectedOrganization.name : 'សរុបកិច្ចប្រជុំតាមក្រសួងស្ថាប័នប្រចាំត្រីមាស' }}
        </div>
        <div class="flex w-full flex-wrap" >
          <table v-if="selectedOrganization.meetings.length > 0 " class="vcb-table border border-gray-300 rounded-lg" >
            ...
            <tr v-for="(record, index) in selectedOrganization.meetings" :key='index' :class=" 'vcb-table-row hover:bg-gray-200 duration-500 ' + ( index % 2 ? ' bg-gray-100 ' : ' bg-gray-50 ' ) " >
          ...
          <div v-if="selectedOrganization.meetings.length > 0 " class="my-4 w-full" >
            <div class="w-1/4 border border-gray-300 bg-gray-100 rounded p-4 my-2 " >ចំនួនម៉ោងប្រើប្រាស់ដើម្បីប្រជុំ ៖ {{ selectedOrganization != null ? ( parseInt( selectedOrganization.totalSpentMinutes / 60 ) + ' ម៉ោង' ) : "0 ម៉ោង" }}</div>
            <div class="w-1/4 border border-gray-300 bg-gray-100 rounded p-4 my-2 " v-if="selectedOrganization != null " >
              ប្រភេទកិច្ចប្រជុំបានឆ្លងកាត់ ៖
              <div class="font-sr ml-4" v-for="(type,index) in selectedOrganization.totalMeetingsByTypes" >{{ (index+1) }}. {{ type.type.name + " ចំនួន " + type.total + " ដង" }}</div>
            </div>
          </div>
...
export default {
  ...
  setup() {
    ...
    function getRecords(){
      store.dispatch('meeting/totalByOrganizationFirstTerm').then( res => {
          if( res.data.ok ){
            listOrganization.value = res.data.result
          }else{
