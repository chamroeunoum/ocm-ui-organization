<template >
  ...
  <div class="flex flex-wrap justify-center pb-4" >
    <div class="flex border-b border-gray-200 pb-2 w-full mb-4" >
      ...
      <div class="flex-grow font-moul text-xs leading-7 text-left ml-1 ">សរុបកិច្ចប្រជុំតាមក្រសួងស្ថាប័ន</div>
    ...
    <n-drawer v-model:show="drawer.show" width="90%" placement="right">
      <n-drawer-content >
        <div class="border-b border-gray-200 pb-4 font-moul mb-4 leading-8" >
          {{  selectedOrganization != null ? selectedOrganization.name : 'សរុបកិច្ចប្រជុំតាមក្រសួងស្ថាប័ន' }}
        </div>
...
export default {
  ...
  setup() {
    ...
    function getRecords(){
      store.dispatch('meeting/totalByOrganizations').then( res => {
          if( res.data.ok ){
