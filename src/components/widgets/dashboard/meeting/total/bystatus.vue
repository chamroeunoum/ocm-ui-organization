<template >
  <!-- Total Meetings -->
  <div @click="$router.push('/meetings')" class="bg-gray-100 rounded border border-gray-200 hover:border-blue-400 duration-300 p-2 m-8 cursor-pointer ">
    <div class="grid grid-cols-1 gap-2" >
      <div class=" flex border-b border-gray-200 pb-2 w-full" >
        ...
        <div class="flex-grow font-moul text-xs leading-7 text-left ml-1 ">ស្ថានភាពកិច្ចប្រជុំ</div>
        <div class="font-bold flex-none w-20 text-right leading-6 text-blue-600 " v-html="total" ></div>
      </div>
      <div class="column-1" >
        <div v-for="(meeting,index) in meetings" :key="index" class="m-auto my-2 font-bold flex justify-between" >
          <div class="pr-2" >{{ getStatus(meeting).label }}</div>
          <div class="pl-2" >{{  meeting.total }}</div>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  setup() {
    ...
    function getRecords(){
      store.dispatch('meeting/totalByStatus').then( res => {
          if( res.data.ok ){
