<template >
  ...
  <div class="flex flex-wrap justify-center pb-4" >
    <div class="flex border-b border-gray-200 pb-2 w-full mb-4" >
      ...
      <div class="flex-grow font-moul text-xs leading-7 text-left ml-1 ">សរុបកិច្ចប្រជុំតាមរយះថ្នាក់ដឹកនាំ</div>
...
export default {
  ...
  setup() {
    ...
    const peopleTitleName = computed( () => {
      return selectedPeople.value != null
        ? ( 
          selectedPeople.value.people.countesies.map( (c) => c.name ).join( " " ) + " " + 
          selectedPeople.value.people.lastname + " " + selectedPeople.value.people.firstname
        )
        : ''
    })
    const peopleOrganization = computed( () => {
      return selectedPeople.value != null
        ? ( 
          selectedPeople.value.people.positions.map( (p) => p.name ).join( " " ) + " " +
          selectedPeople.value.people.organizations.map( (o) => o.name ).join( " " )
        )
        : ''
    })
    ...
    function getRecords(){
      store.dispatch('meeting/totalByTypeByLeaders').then( res => {
          if( res.data.ok ){
