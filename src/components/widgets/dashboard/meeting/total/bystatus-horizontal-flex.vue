<template >
  ...
  <div class="flex flex-wrap justify-center pb-4" >
    <div class="flex border-b border-gray-200 w-full mb-4" >
      ...
      <div class="flex-grow font-moul text-xs leading-7 text-left ml-1 ">សរុបកិច្ចប្រជុំតាមស្ថានភាព</div>
    ...
    <div v-for="(meeting,index) in meetings" :key="index" @click="$router.push('/meetings')" 
      :class="'flex flex-wrap justify-center border border-gray-200 hover:border-blue-400 duration-300 m-4 p-8 rounded cursor-pointer font-bold ' + ( getStatus(meeting).color ) " 
    >
      <div class="w-full text-center mb-4 " >{{ getStatus(meeting).label }}</div>
      <div class="w-full text-center" >{{  meeting.total }}</div>
    </div>
  </div>
  <!-- End Total Meetings -->
</template>
