<template >
  ...
  <div class="flex flex-wrap justify-center pb-4" >
    <div class="flex border-b border-gray-200 pb-2 w-full mb-4" >
      ...
      <div class="flex-grow font-moul text-xs leading-7 text-left ml-1 ">សរុបកិច្ចប្រជុំតាមរយះថ្នាក់ដឹកនាំត្រីមាស</div>
    ...
    <n-drawer v-model:show="drawer.show" width="90%" placement="right">
      <n-drawer-content title="សរុបកិច្ចប្រជុំតាមរយះថ្នាក់ដឹកនាំត្រីមាស">
...
export default {
  ...
  setup() {
    ...
    function getRecords(){
      store.dispatch('meeting/totalByTypeByLeaderFirstTerm').then( res => {
          if( res.data.ok ){
