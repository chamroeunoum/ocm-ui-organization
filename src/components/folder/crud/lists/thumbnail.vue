  ...
  <div class="relative" >
          ...
          <n-tooltip trigger="hover">
            <template #trigger>
              <div class="w-full relative" >
                <input type="text" @keypress.enter="filterRecords(false)" v-model="search" class="bg-gray-100 px-2 h-8 w-full rounded border border-gray-200 focus:border-blue-600 hover:border-blue-600 duration-300" placeholder="ស្វែងរក" />
                <svg class="absolute right-1 top-1 w-6 h-6 text-gray-400  cursor-pointer" @click="filterRecords(false)" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 20 20"><g fill="none"><path d="M8.5 3a5.5 5.5 0 0 1 4.227 9.02l4.127 4.126a.5.5 0 0 1-.638.765l-.07-.057l-4.126-4.127A5.5 5.5 0 1 1 8.5 3zm0 1a4.5 4.5 0 1 0 0 9a4.5 4.5 0 0 0 0-9z" fill="currentColor"></path></g></svg>
              </div>
            </template>
    ...
    <div class="vcb-table-panel relative h-screen ">
      <Transition name="slide-fade" >
        <div v-if="Array.isArray( records.matched ) && records.matched.length > 0 " class="vcb-thumbnail mb-12" >
            ...
            <div class="content " >
              ...
              <div class="flex flex-wrap " >
                <div class="w-full pt-2 pb-4" >
                  <div class="w-full text-center font-moul mx-auto mb-4" >{{  record.name }}</div>
                  <div class="w-full text-center leading-6 tracking-wider" >{{ record.desp }}</div>
                </div>
              </div>
      ...
      <!-- Pagination of crud -->
      <Transition name="slide-fade" >
        <div v-if="pagination.totalPages > 1 " class="fixed left-0 right-0 bottom-1 flex flex-wrap" >
...
export default {
  ...
  setup(props){
    ...
    const crud = new Crud()
    crud.setConfig( import.meta.env.VITE_API_SERVER , store.getters[props.model.name+'/model'] , store.getters[props.model.name+'/columns'].all )

    const server = computed( () => {
      return store.getters[ props.model.name + '/server']
    })
    const pagination = computed(()=>{
      return store.getters[ props.model.name+ '/pagination']
    })
    const columns = computed( () => {
      return store.getters[ props.model.name + '/columns']
    })

    const records = computed( () => {
      return store.getters[ props.model.name + '/records' ]
    })
    ...
    function previous(){
      crud.previous( 
        ...
        function(data){
          store.commit( props.model.name + '/setRecords' , data )
        }, 
